...
namespace SimpleHealthBar_SpaceshipExample
{
	public class GameManager : MonoBehaviour
	{
		...
		void SpawnHealthPickup ()
		{
			...
			// CreateDamage a Vector3 varaible to store the spawn position.
			...
			// CreateDamage the health pickup game object at the position( determined above ), and at a random rotation.
		...
		void SpawnAsteroid ()
		{
			...
			// CreateDamage the asteroid game object at the position( determined above ), and at a random rotation.
		...
		public void SpawnDebris( Vector3 pos )
		{
			...
			{
				// CreateDamage a force to make the debris fly away from eachother.
				Vector3 force = Quaternion.Euler( 0, i * 360f / numberToSpawn, 0 ) * Vector3.forward * 5.0f * 300f;

				// CreateDamage the new debris game object at the asteroid's position, plus the forces position to make sure that the debris is positioned correctly. Random rotation as well.
		...
		{
			// CreateDamage a new explosion prefab game object at the desired position, with default rotation.
		...
		IEnumerator FadeDeathScreen ()
		{
			...
			// CreateDamage temporary colors to be able to apply a fade to the image and text.
